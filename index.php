...
<body>
        ...
        <div class="tab-pane fade show active" id="nav-busin" role="tabpanel" aria-labelledby="nav-busin-tab"
            ...
            <div class="bg-body-tertiary border rounded-3">
                <table class="table caption-top">
                    <caption>
                        <button onclick="clearForm()" id="openModal" type="button" class="btn btn-primary"
                            data-bs-toggle="modal" data-bs-target="#staticBackdrop">Добавить</button>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <script type="text/javascript">
                                            function Accept() {
                                                ...
                                                saveIncident(model, name, price, description, id)
                                            }
                                        </script>
                                    </div>
                                </div>
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            echo "<tr><th scope='row'>" . $row['id_busin']
                                ...
                                . '<td class="icons"> <svg onclick="editIncident(\'' . $row['id_busin'] . '\',\'' . $row['model_busin'] . '\',\'' . $row['name_busin'] . '\',\'' . $row['price_busin'] . '\',\'' . $row['description_busin'] . '\')" xmlns="http://www.w3.org/2000/svg" width="20" height="20" 
                                ...
                                </svg> <svg onclick="deleteIncident(\'' . $row['id_busin'] . '\')" xmlns="http://www.w3.org/2000/svg" valign="middle" width="20" height="20" 
                                fill="currentColor" 
    ...
    <script>
        ...
        function editIncident(id, model, name, price, description) {
            $('#id')[0].value = id;
            $('#model')[0].value = model;
            $('#name')[0].value = name;
            $('#price')[0].value = price;
        ...
        function saveIncident(model, name, price, description, id = "") {
            $.ajax({
                url: "script_accept_busin.php",
                type: "POST",
                data: "&model=" + model + "&name=" + name + "&price=" + price + "&description=" + description + "&id=" + id,
                success: function () {
                    location.reload();
                }
            });
